...
class SatelliteDataset(Dataset):
  ...
  def __init__(self, root_dir, name_list, images_dir = "images", labels_dir = "labels", type_ = "train", channels = 3, transform = None, label_axis = -1):
    ...
    try:
      ...
      raise ValueError(f"Type of dataset needs to be train/test/media, not '{self.type_}'")
  ...
  def __getitem__(self, idx):
    ...
    if self.type_ is not "media":
      label_name = os.path.join(self.root_dir, self.labels_dir, self.name_list[idx])
    ...
    sample = {"image": image, "label": label} if self.type_ is not "media" else image
...
def trainEpoch(model, epoch, optimizer, dataloader, num_steps, loss_fn):
    ...
    for step, batch in enumerate(dataloader):
      ...
      string = "Epoch: {}   Step: {}   Batch Loss: {:.4f}   Epoch Loss: {:.4f}   Epoch ETA: {}".format(epoch,
                    step,
                    loss,
                    epoch_loss.mean(),
                    ETA
...
def testModel(model, epoch, dataloader, num_steps, test_metric):
    ...
    for step, batch in enumerate(dataloader):
      ...
      string = "Evaluating epoch: {}   Step: {}   Batch score: {:.4f}   Epoch score: {:.4f}   Epoch ETA: {}".format(epoch,
                    step,
                    score,
                    epoch_score.mean(),
                    ETA
...
def train_test_dataset(data_dir, test_size=0.3, train_size=None, images_dir = "images", labels_dir = "labels", transform = None, random_state=None):
  ...
  if random_state:
    train_list, test_list = train_test_split(name_list, test_size = test_size, train_size = train_size, random_state = random_state)
  else:
    train_list, test_list = train_test_split(name_list, test_size = test_size, train_size = train_size)
 
  print(f"{len(train_list)} training images, {len(test_list)} testing images")
 
  train_dataset = SatelliteDataset(data_dir, train_list, type_ = "train", channels = 3, transform = transform)
  test_dataset = SatelliteDataset(data_dir, test_list, type_ = "test", channels = 3, transform = None)
...
def produceImage(model, epoch, media_dir_name, data_dir, video = False, figsize = 30):
  ...
  dataset = SatelliteDataset(root_dir = f"{data_dir}/mediaset", name_list=name_list, type_ = "media")
  ...
  wandb.log({"Examples images": wandb.Image(f"media/{media_dir_name}/epoch_{epoch}.png")})
...
def loadRecentModel(save_path):
  ...
  if len(saves) == 0:
    ...
    try:
      ...
      raise Exception(f"Invalid file name for recent model {recent_model}, should be of format epoch_#")
